...
public struct CasinoHubView: View {
    ...
    // These raw IDs are an auditable routing contract for all 11 tables.
    private let routeIDs = ["blackjack", "five-card-draw", "red-black", "higher-lower", "high-card", "coin-call", "dice-duel", "over-under-seven", "odd-even", "fair-wheel", "number-draw"]

    public init(
        previewSeed: UInt64? = nil,
        entryAccessPolicy: CasinoEntryAccessPolicy = .practiceOnly
    ) {
    ...
    public var body: some View {
        Group {
            if entryAccessStatus.canEnterCasino {
                ...
                CasinoEntryGateView(
                    onEnterPractice: {
                        entryAccessStatus = entryAccessPolicy.enterPracticeSession()
                    },
                    onLeave: { dismiss() }
                )
            }
        ...
        .alert("Reset Session", isPresented: $showingResetConfirmation) {
            ...
            Text("Clear compact and Five-Card Draw results from this visit? This does not clear the existing Blackjack audit save.")
        }
        .task(id: entryAccessStatus) {
            guard entryAccessStatus.canEnterCasino else { return }
            await session.restoreOrDeal()
            if casinoSession.selectedGameID != .blackjack, session.loadState == .ready {
                session.endHand()
            }
        }
        .onChange(of: scenePhase) { _, phase in
            guard entryAccessStatus.canEnterCasino, phase != .active else { return }
            Task { await session.persist() }
        }
    }

    private var casinoHub: some View {
        VStack(spacing: 0) {
            hubHeader
            Divider()
            GeometryReader { proxy in
                ...
                case .regular(let sidebarWidth):
                    regularHubLayout(sidebarWidth: sidebarWidth)
                }
            }
        }
    ...
    private var hubHeader: some View {
        VStack(alignment: .leading, spacing: 10) {
            HStack(alignment: .center) {
                ...
                Button { dismiss() } label: {
                    Label("Leave Game", systemImage: "door.left.hand.open")
        ...
        .background {
            ZStack(alignment: .trailing) {
                ...
                CasinoProbabilityRosette(style: .watermark, diameter: 126)
                    .offset(x: 22)
    ...
    private var casinoLibrary: some View {
        VStack(alignment: .leading, spacing: 8) {
            Text("Tables").font(.headline).foregroundStyle(.primary)
            ForEach(PrismetPracticeCasinoCatalog.all) { game in
                gameChoice(game)
            }
        }
        .casinoPanel()
    ...
    @ViewBuilder private var tableSurface: some View {
        VStack(alignment: .leading, spacing: 12) {
            tableHeader
            if casinoSession.selectedGameID == .blackjack {
                PracticeBlackjackView(session: session)
            } else if casinoSession.selectedGameID == .fiveCardDraw {
                PracticePokerView(session: casinoSession)
            } else {
                PracticeChanceGameView(session: casinoSession)
            }
        }
        .frame(
            maxWidth: .infinity,
            maxHeight: casinoSession.selectedGameID == .blackjack ? .infinity : nil,
            alignment: .top
        )
    }
    ...
    private var tableHeader: some View {
        VStack(alignment: .leading, spacing: 4) {
            Text(casinoSession.descriptor.title)
                .font(.title2.bold())
                .foregroundStyle(.primary)
            Text(casinoSession.descriptor.subtitle)
    ...
    private func regularHubLayout(sidebarWidth: CGFloat) -> some View {
        HStack(alignment: .top, spacing: CasinoMobileLayoutPolicy.hubSpacing) {
            ScrollView { casinoLibrary }
                .frame(width: CasinoMobileLayoutPolicy.libraryWidth)
            regularTableRegion
                .frame(maxWidth: .infinity, maxHeight: .infinity)
            ScrollView { rulesInspector }
            ...
            ScrollView {
                ZStack {
                    RoundedRectangle(cornerRadius: 24, style: .continuous)
                        .fill(CasinoTheme.feltBackground)
                    CasinoProbabilityRosette(style: .watermark, diameter: 286)
                        .opacity(0.72)
                        .accessibilityHidden(true)
                    tableSurface
                        .padding(12)
                }
                .frame(minHeight: CasinoMobileLayoutPolicy.regularTableMinimumHeight)
            }
        }
    }
    ...
    private var rulesInspector: some View {
        VStack(alignment: .leading, spacing: 12) {
            ...
            Text("Reset clears only compact and Five-Card Draw visit results. It does not clear the existing Blackjack audit save.").font(.footnote).foregroundStyle(.secondary)
        }
        .casinoPanel()
    }
    ...
    private func gameChoice(_ game: PrismetPracticeCasinoGameDescriptor) -> some View {
        ...
        .disabled(isBlackjackSwitchBlocked && game.id != .blackjack)
    }
