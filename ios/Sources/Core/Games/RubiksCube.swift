...
enum CubeFace: String, CaseIterable, Codable, Hashable {
    ...
    /// Human-readable name for the face, e.g. "Up (top)". Used by the on-screen
    /// legend and the "How to play" help so U/D/L/R/F/B are self-explanatory.
    var plainName: String {
        switch self {
        case .U: return "Up (top)"
        case .D: return "Down (bottom)"
        case .L: return "Left"
        case .R: return "Right"
        case .F: return "Front"
        case .B: return "Back"
...
enum RubiksMove: String, CaseIterable, Codable, Hashable, Identifiable {
    ...
    /// `true` when the move is a counter-clockwise (prime) quarter turn — the ' moves.
    var isPrime: Bool {
        switch self {
        case .Uprime, .Dprime, .Lprime, .Rprime, .Fprime, .Bprime: return true
        default: return false
        }
    }

    /// Turn direction in plain words, matching the standard cube convention:
    /// a plain move (e.g. "R") is clockwise, a prime move ("R'") is
    /// counter-clockwise, and a double move ("R2") is a half turn.
    var directionDescription: String {
        switch quarters {
        case 2: return "half turn"
        case 3: return "counter-clockwise"
        default: return "clockwise"
        }
    }

    /// Full plain-English reading of the move, e.g. `R'` -> "Right face,
    /// counter-clockwise". Drives the accessibility label and the legend so the
    /// U/D/L/R/F/B + ' notation is self-explanatory.
    var spokenDescription: String {
        "\(face.plainName) face, \(directionDescription)"
    }
    ...
    static let mobileControlRows: [RubiksMoveControlRow] = [
        RubiksMoveControlRow(face: .U, turn: .U, inverse: .Uprime),
        RubiksMoveControlRow(face: .D, turn: .D, inverse: .Dprime),
        RubiksMoveControlRow(face: .L, turn: .L, inverse: .Lprime),
        RubiksMoveControlRow(face: .R, turn: .R, inverse: .Rprime),
        RubiksMoveControlRow(face: .F, turn: .F, inverse: .Fprime),
        RubiksMoveControlRow(face: .B, turn: .B, inverse: .Bprime)
    ]
...
struct RubiksMoveControlRow: Hashable, Identifiable {
    var face: CubeFace
    var turn: RubiksMove
    var inverse: RubiksMove

    var id: CubeFace { face }
}
