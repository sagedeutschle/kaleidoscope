...
struct SolitaireGame: Codable, Hashable {
    ...
    /// Total cards already sent home across all four foundations (0…52). Drives the
    /// UI's completion progress + the celebration trigger for the auto-finish cascade.
    var foundationCardCount: Int {
        Suit.allCases.reduce(0) { $0 + (foundations[$1]?.count ?? 0) }
    }

    /// Fraction of the deck home on the foundations, 0…1. `1` exactly when `isWon`.
    var completionProgress: Double {
        Double(foundationCardCount) / Double(Suit.allCases.count * Rank.allCases.count)
    }
    ...
    /// Number of cards still cyclable through the stock + waste.
    var stockPlusWasteCount: Int { stock.count + waste.count }

    /// True once every tableau card is face-up. At that point Klondike (draw-1) is
    /// always winnable, so the game can finish itself.
    var allTableauFaceUp: Bool {
        tableau.allSatisfy { pile in pile.allSatisfy(\.isFaceUp) }
    }

    /// The game should auto-complete: everything is exposed but it isn't won yet.
    var canAutoFinish: Bool { allTableauFaceUp && !isWon }
    ...
    /// Perform exactly ONE foundation move — the waste top first, then the first
    /// eligible tableau top. Returns whether it moved. This is the unit step the UI
    /// drives (with a delay between calls) to animate the auto-finish cascade.
    @discardableResult
    mutating func autoStepToFoundation() -> Bool {
        if moveWasteToFoundation() { return true }
        for index in tableau.indices where moveTableauToFoundation(pile: index) { return true }
        return false
    }
