...
struct Piece: Hashable, Codable {
    ...
    /// Unicode chess glyph matching the piece color (white = outline, black =
    /// filled). Kept for anywhere a color-accurate single glyph is wanted.
    var glyph: String {
        switch (color, type) {
    ...
    /// Always the SOLID (filled) glyph shape (U+265A–F), regardless of color, so
    /// both sides render as solid pieces we tint ourselves — white pieces are
    /// solid fills, not the thin outlines the white Unicode glyphs would give.
    var solidGlyph: String {
        switch type {
        case .king:   return "♚"
        case .queen:  return "♛"
        case .rook:   return "♜"
