...
- **Owns:** NEW `shared/PrismetShared/Sources/PrismetShared/PrismetPracticeCasinoCatalog.swift`, `PrismetFairChanceEngine.swift`, `PrismetFiveCardPoker.swift` and matching package tests; EDIT `ios/Sources/Features/Casino/CasinoHubView.swift`, `CasinoTheme.swift`, `CasinoSafetyContractTests.swift`, `CasinoMobilePresentationTests.swift`; NEW iOS Casino session/game/poker views; the matching macOS Casino hub/theme/tests and new session/game/poker views; this ledger plus the expansion spec/plan.
- **Excludes:** Home/root/facet registration, Catan, all non-Casino games, ads/purchases, account/cloud/Game Center/leaderboards, both `project.yml` files, generated projects, version/build numbers, App Store upload/submission, and the pre-existing untracked `docs/superpowers/plans/2026-07-14-prismet-practice-blackjack.md`.
